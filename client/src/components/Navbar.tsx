...
const Navbar = () => {
     ...
     const [credits,setCredits]=React.useState(0)
      ...
      console.log("React session:", session);
  ...
  return (
    ...
    <nav className="z-50 flex items-center justify-between w-full py-4 px-4 md:px-16 lg:px-24 xl:px-32 backdrop-blur border-b text-white border-slate-800">
        <Link to="/">
             <img src={assets.logo} alt="logo" className='h-7 sm:h-7 '
             />
          </Link>
  ...
  <Link to="/" className="relative group">
    <span className="transition duration-300 group-hover:text-indigo-400">
      Home
    </span>
  ...
  <Link to="/pricing" className="relative group">
    ...
    <span className="absolute left-0 -bottom-1 h-[2px] w-0 bg-indigo-400 transition-all duration-300 group-hover:w-full"></span>
        ...
        {/* Mobile Menu */}
        {menuOpen && (
          <div className="fixed inset-0 z-[100] bg-black/50 text-white backdrop-blur flex flex-col items-center justify-center text-lg gap-8 md:hidden transition-transform duration-300">
